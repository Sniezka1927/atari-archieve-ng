...
export class CzasopismaService {
  ...
  private readonly czasopismoKeys: (keyof Czasopismo)[] = [
    'miniaturka',
    'format',
    'stron',
    'nazwa',
  ...
  async fetchCzasopismaInfo(): Promise<CzasopismaInfo[]> {
    ...
    for (const header of this.headers) {
      ...
      if (categories instanceof Error) {
        throw new Error(
          `Category error when fetching categories and headers ${categories.message}`
        );
      }
  ...
  async fetchCategories(headerName: string): Promise<string[] | Error> {
    ...
    try {
      response = this.czasopismaXml.evaluate(
        `/czasopisma/lata/${headerName}`,
        this.czasopismaXml,
        null,
    ...
    if (node === null) {
      return new Error(`Node is null for header ${headerName}`);
    }
    return (node.textContent as string).split(',');
  }

  async fetchCzasopisma(
    headerName: string,
    category: string
  ): Promise<Czasopismo[]> {
    if (!this.isInitialized) {
    ...
    try {
      response = this.czasopismaXml.evaluate(
        `/czasopisma/${headerName}/*[@rok='${category}' and not(boolean(@brak))]`,
        this.czasopismaXml,
        null,
        XPathResult.ANY_TYPE
      );
  ...
  private initializeHeaders(): Promise<void> {
    return new Promise(async (resolve) => {
      const response = this.czasopismaXml.evaluate(
        `/czasopisma/zmienne`,
        this.czasopismaXml,
        null,
        XPathResult.ANY_TYPE
